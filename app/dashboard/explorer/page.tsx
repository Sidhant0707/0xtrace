...
// ============================================================================
// Explorer - Raw LLM Call Browser (v2 Multi-Tenant)
// ============================================================================
// CRITICAL CHANGE: All queries now filtered by project_id from active project.
//
// Every individual llm_call row, filterable, sortable, and paginated.
...
import { getActiveProjectId } from "@/lib/project-context"; // ← NEW: Project scoping
...
// ── Data fetching (Scoped to Project) ────────────────────────────────────────

async function getCalls(
  projectId: string, // ← NEW: Required parameter
  filters: {
    model?: string;
    q?: string;
    status?: string;
...
export default async function ExplorerPage({ searchParams }: PageProps) {
  // ── Get active project ID (validates ownership via RLS) ──────────────────
  ...
  // ── Fetch data scoped to project ──────────────────────────────────────────
  const [calls, models] = await Promise.all([
    getCalls(projectId, { model, q, status }),
      ...
      <div className="flex items-end justify-between gap-6 mb-6">
        <div>
          ...
          <p className="mt-1.5 text-[#71717a] text-sm m-0">
            {calls.length} call{calls.length !== 1 ? "s" : ""} · last 7 days
          </p>
        </div>
      </div>

      {/* ── Explorer table (client component for interactivity) ── */}
      <ExplorerTable
        calls={calls}
        models={models}
        initialModel={model}
